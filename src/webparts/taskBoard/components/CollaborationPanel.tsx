...
const Avatar: React.FC<{ collaborator: ICollaborator; size?: number }> = ({ collaborator, size = 30 }) => (
    <div title={`${collaborator.name}${collaborator.email ? ` — ${collaborator.email}` : ''}`} style={{
...
const StatusBadge: React.FC<{ status: ICollaborationRequest['status'] }> = ({ status }) => {
    const colorMap = { Pending: '#f59e0b', Accepted: '#22c55e', Declined: '#ef4444' };
    return (
        <span style={{ fontSize:'10px', fontWeight:700, color:'#ffffff', backgroundColor:colorMap[status], borderRadius:'999px', padding:'2px 8px', textTransform:'uppercase', letterSpacing:'0.3px', flexShrink:0 }}>
            {status}
        </span>
    );
};

const CollaborationPanel: React.FC<ICollaborationPanelProps> = ({ taskSpId, taskTitle, currentUserSpId }) => {
    ...
    const handleCancelRequest = async (requestId: number, collaboratorName: string) => {
    ...
    if (!taskSpId) {
        return (
            <div style={sectionWrapperStyle}>
                ...
                <div style={{ fontSize:'13px', color:THEME.colors.textSecondary }}>Save the task first before adding collaborators.</div>
            </div>
        );
    }
    ...
    return (
        <div style={sectionWrapperStyle}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                <div style={sectionLabelStyle}>In Collaboration With</div>
                <button type="button" onClick={handleToggleRequesting} style={{ fontSize:'12px', fontWeight:600, color: isRequesting ? THEME.colors.textSecondary : '#2563eb', background:'none', border:'none', cursor:'pointer', padding:'2px 6px' }}>
                    {isRequesting ? 'Cancel' : '+ Request Collaborator'}
                </button>
            </div>

            {isLoading && <div style={{ fontSize:'13px', color:THEME.colors.textSecondary }}>Loading...</div>}
            ...
            {!isLoading && acceptedCollaborators.length === 0 && !isRequesting && (
                <div style={{ fontSize:'13px', color:THEME.colors.textSecondary, marginBottom:'12px' }}>No collaborators yet.</div>
            )}

            {isRequesting && (
                <div style={requestFormStyle}>
                    <div style={{ fontSize:'12px', fontWeight:600, color:THEME.colors.textSecondary, marginBottom:'6px' }}>Search for a person to collaborate with</div>
                    <PeoplePicker value={selectedUser} onChange={setSelectedUser} placeholder="Search by name or email..." canEdit={true} />
                    <button type="button" onClick={handleSendRequest} disabled={!selectedUser || isSending} style={{ marginTop:'10px', width:'100%', padding:'10px', borderRadius:'8px', border:'none', backgroundColor: !selectedUser || isSending ? '#94a3b8' : '#2563eb', color:'#ffffff', fontSize:'14px', fontWeight:600, cursor: !selectedUser || isSending ? 'not-allowed' : 'pointer', transition:'background-color 150ms ease' }}>
                        {isSending ? 'Sending...' : 'Send Collaboration Request'}
                    </button>
                </div>
            )}

            {successMessage && <div style={{ ...feedbackStyle, backgroundColor:'#f0fdf4', color:'#15803d', borderColor:'#bbf7d0' }}>{successMessage}</div>}
            {errorMessage && <div style={{ ...feedbackStyle, backgroundColor:'#fef2f2', color:'#dc2626', borderColor:'#fecaca' }}>{errorMessage}</div>}
                        ...
                        {pendingRequests.map(req => (
                            <div key={req.requestId} style={requestRowStyle}>
                                ...
                                <div style={{ flex:1, minWidth:0 }}>
                                    <div style={{ fontSize:'13px', fontWeight:600, color:THEME.colors.textPrimary }}>{req.collaborator.name}</div>
                                    <div style={{ fontSize:'11px', color:THEME.colors.textSecondary }}>Requested {formatDate(req.requestedAt)} by {req.requestedBy.name}</div>
                                </div>
                                <StatusBadge status={req.status} />
                                {req.requestedBy.id === currentUserSpId && (
                                    <button type="button" onClick={() => handleCancelRequest(req.requestId, req.collaborator.name)} title="Cancel request" style={cancelButtonStyle}>&times;</button>
                                )}
                            </div>
                        ))}
                    </div>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'6px', marginTop:'8px' }}>
                        {declinedRequests.map(req => (
                            <div key={req.requestId} style={{ ...requestRowStyle, opacity:0.65 }}>
                                ...
                                <div style={{ flex:1, minWidth:0 }}>
                                    <div style={{ fontSize:'13px', fontWeight:600, color:THEME.colors.textPrimary }}>{req.collaborator.name}</div>
                                    <div style={{ fontSize:'11px', color:THEME.colors.textSecondary }}>Declined {formatDate(req.respondedAt)}</div>
                                </div>
                                <StatusBadge status={req.status} />
                            </div>
                        ))}
...
const sectionWrapperStyle: React.CSSProperties = { borderTop: `1px solid ${THEME.colors.border}`, paddingTop: '16px', marginTop: '4px' };
const sectionLabelStyle: React.CSSProperties = { fontSize: '11px', fontWeight: 600, color: THEME.colors.textSecondary, textTransform: 'uppercase', letterSpacing: '0.4px' };
const requestFormStyle: React.CSSProperties = { backgroundColor: '#f8fafc', border: `1px solid ${THEME.colors.border}`, borderRadius: '10px', padding: '12px', marginBottom: '10px' };
const requestRowStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 10px', backgroundColor: '#f8fafc', border: `1px solid ${THEME.colors.border}`, borderRadius: '8px' };
const feedbackStyle: React.CSSProperties = { fontSize: '13px', padding: '10px 12px', borderRadius: '8px', border: '1px solid', marginTop: '8px' };
const cancelButtonStyle: React.CSSProperties = { background: 'none', border: 'none', color: THEME.colors.textSecondary, fontSize: '18px', cursor: 'pointer', lineHeight: 1, padding: '0 2px', flexShrink: 0 };
